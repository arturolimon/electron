...
Adafruit_LIS3DH accel = Adafruit_LIS3DH(A2, A5, A4, -1);

AssetTracker::AssetTracker(Stream * eout){
    errorOut = eout;
}
...
void AssetTracker::gpsOn(){
    ...
    mySerial.begin(9600);
    gps.sendCommand(PMTK_SET_NMEA_OUTPUT_RMCONLY);//PMTK_SET_NMEA_OUTPUT_RMCGGA);
    delay(500);
...
void AssetTracker::updateGPS(){
  ...
  if (gps.newNMEAreceived()) {
    ...
    //Serial.println(gps.peekNMEA());   // this also sets the newNMEAreceived() flag to false
...
volatile char gpsBuffer[MAXLINE][MAXLINELENGTH];

// our index into filling the current line
volatile uint8_t lineidx    = 0;
volatile uint8_t lineRidx   = 0;
volatile uint8_t lineWidx   = 0;
volatile uint8_t charRidx   = 0;
volatile uint8_t charWidx   = 0;
...
boolean Adafruit_GPS::parse(char *nmea) {
  ...
  if (nmea[strlen(nmea)-4] == '*') {
    uint16_t receivedSum = parseHex(nmea[strlen(nmea)-3]) * 16;
    receivedSum += parseHex(nmea[strlen(nmea)-2]);

    // check checksum
    for (uint8_t i=1; i < (strlen(nmea)-4); i++) {
      calculatedSum ^= nmea[i];
    }
    if (calculatedSum != receivedSum) {
      // bad checksum :(
      errorOut->printf("Bad sum %u %u\n\r", receivedSum, calculatedSum);
      return false;
    }
  }
  else
  {
      errorOut->printf("No sum\n\r");
  }
...
char Adafruit_GPS::read2(void) {
    ...
    for (unsigned int i = 0; i < charAvail; i++)
    {
        c = gpsHwSerial->read();

        // End of line. Wrap up this line and continue with next char
        if (c == '\n')
        {
            gpsBuffer[lineWidx][charWidx] = 0;

            advanceWriteLine();
            charWidx = 0;
            continue;
        }
        else if (c == '$') // Start of line. Make sure we write at the beginning of the line
        {
            gpsBuffer[lineWidx][charWidx] = 0;
            charWidx = 0;
        }

        gpsBuffer[lineWidx][charWidx++] = c; // Write received char at the right place

        /* Hacky thing here. If we're going out of buffer bounds then just keep overwriting
           the last slot of the buffer until we get a end of line.
           This guarantees that we don't end up with sentences split over two buffer lines. */
        if (charWidx >= MAXLINELENGTH)
            charWidx = MAXLINELENGTH-1;
    }

    return c; // This will end up returning false if we didn't read anything
}

void Adafruit_GPS::advanceWriteLine(void)
{
    //make sure last char is a 0 so as to avoid buffer overrun
    gpsBuffer[lineWidx][MAXLINELENGTH-1] = 0;

    if (lineWidx == lineRidx)
        recvdflag = true;

    lineWidx = (lineWidx + 1) % MAXLINE;

}

void Adafruit_GPS::advanceReadLine(void)
{
    lineRidx = (lineRidx + 1) % MAXLINE;

    if (lineRidx == lineWidx)
        recvdflag = false;
}

Adafruit_GPS::Adafruit_GPS(Stream *ser, Stream *eout) {
...
char *Adafruit_GPS::lastNMEA(void) {
  char * nmea = (char *) gpsBuffer[lineRidx];
  advanceReadLine();
  return nmea;
}

char *Adafruit_GPS::peekNMEA(void) {
  char * nmea = (char *) gpsBuffer[lineRidx];
  return nmea;
}
